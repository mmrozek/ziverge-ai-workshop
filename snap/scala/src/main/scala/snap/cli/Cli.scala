...
object Command:
  ...
  /** Commands resolved against the nearest repository before dispatch (R77). `init` creates a
    ...
    * filesystem at all (R91, DESIGN §10 gotcha 9) — every other command needs one. `config`'s
    * `--global` exemption (DESIGN D10) is a finer distinction than this stub-era dispatch makes;
    * T09, which implements `config`, refines this (see T08's Notes / decisions).
    */
  def needsRepoDiscovery(c: Command): Boolean = c match
    case Init | Version => false
    case _              => true

/** A handler for one command: given the effect boundary and the discovered repository root (`None`
  * when the command doesn't need one, per [[Command.needsRepoDiscovery]]), produce either the
  * command's stdout text or a [[SnapError]]. Every command in the map has this exact shape, which
  * is what lets later tasks swap a stub for a real implementation without touching [[Cli.run]].
  */
type CommandHandler = (Env, Option[Path]) => Either[SnapError, String]
...
object Cli:
  ...
  private val stub: CommandHandler = (_, _) => Left(SnapError.NotImplemented)
  ...
  val defaultCommands: Map[Command, CommandHandler] =
    Command.values.iterator.filterNot(_ == Command.Version).map(c => c -> stub).toMap
    ...
    validateSnapColor(env) match
      ...
      case Right(()) =>
        parse(args) match
          ...
          case Right(ParsedCommand(cmd, _))             =>
            val outcome =
              (if Command.needsRepoDiscovery(cmd) then discoverRepo(env).map(Some.apply)
               else Right(None))
                .flatMap(root => commands(cmd)(env, root))
            emit(env, outcome)
